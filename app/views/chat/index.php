...
.chat-back-link {
  ...
  .msg-bubble-content { display: flex; flex-direction: column; gap: 4px; max-width: min(42%, 380px); min-width: 0; }
  .msg-row.mine .msg-bubble-content { align-items: flex-end; }
  .bubble { display: inline-block; min-width: 50px; padding: 9px 12px; border-radius: 16px; font-size: 14px; line-height: 1.4; word-break: break-word; overflow-wrap: break-word; white-space: pre-wrap; max-width: 100%; }
  ...
  @media (max-width: 768px) {
    ...
    .chat-messages-area { padding:14px 14px 156px; gap:12px; }
    .msg-row { gap:6px; }
    .msg-avatar { width:32px; height:32px; }
    .msg-bubble-content { max-width: calc(100% - 40px); }
    .bubble { font-size:13px; line-height:1.4; padding:9px 12px; }
    ...
    .chat-text-input { min-height:40px; max-height:110px; padding:10px 12px; font-size:16px; line-height:1.35; border-radius:14px; }
  ...
  @media (max-width: 480px) {
    .chat-messages-area { padding: 12px 12px 164px; }
    ...
    <div class="new-chat-section">
      ...
      <div class="all-users-list">
        <?php foreach ($allUsers as $u): ?>
          ...
          <?php endif; ?>
        <?php endforeach; ?>
        <?php if (empty($allUsers)): ?><div style="font-size:12px; color:var(--text-dim); padding:8px;">No other users yet.</div><?php endif; ?>
      </div>
    </div>
  </div>

  <div class="chat-content-wrap <?= $otherUser ? 'with-info' : 'single-pane' ?>">
    ...
    <?php if ($otherUser): ?>
        ...
        <?php else: ?>
          <?php $lastMsgId = 0; foreach ($messages as $msg): $lastMsgId = max($lastMsgId, (int)$msg['id']); $isMine = ((int)$msg['sender_id'] === (int)$currentUserId); ?>
            <div class="msg-row <?= $isMine ? 'mine' : '' ?>" id="msg-<?= (int)$msg['id'] ?>">
              ...
              <?php if ($isMine): ?><img decoding="async" loading="lazy" src="<?= avatarUrlC($currentAvatar, $currentFullName) ?>" class="msg-avatar" alt=""><?php endif; ?>
            </div>
          <?php endforeach; ?>
        <?php endif; ?>
      ...
      <div class="chat-input-area" id="chatInputArea">
        ...
        <div class="chat-composer-main">
          <textarea class="chat-text-input" id="msgInput" rows="1" data-autogrow="true" data-max-height="144" placeholder="Message <?= htmlspecialchars(explode(' ', trim($otherUser['full_name'] ?? ''))[0] ?? 'them', ENT_QUOTES, 'UTF-8') ?>…"></textarea>
...
<script>
  ...
  function syncComposerState() {
    ...
    chatInputArea?.classList.toggle('is-composing', hasText || hasAttachment || isFocused);
...
async function toggleVoiceRecording() {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (emojiPicker.contains(event.target) || emojiToggleBtn.contains(event.target)) return;
  ...
  async function sendMsg() {
    ...
    try {
      ...
      if (data.message_type === 'image') {
        appendImageMsg(data.media_file, true, data.time, data.id, myAvatar, !!data.is_read);
      } else if (data.message_type === 'voice') {
        appendVoiceMsg(data.media_file, data.media_duration || 0, true, data.time, data.id, myAvatar, !!data.is_read);
      } else {
        appendTextMsg(data.message, true, data.time, data.id, myAvatar, !!data.is_read);
      }
  ...
  function insertMessageRow(innerHtml, isMine, time, id, avatarSrc, isRead = false) {
    const area = document.getElementById('messagesArea');
  ...
  function appendTextMsg(text, isMine, time, id, avatarSrc, isRead = false) {
    const normalized = decodeHtmlEntities(String(text));
    const escapedText = escapeHtml(normalized).replace(/\n/g, '<br>');
    insertMessageRow(`<div class="bubble ${isMine ? 'mine' : 'theirs'}">${escapedText}</div>`, isMine, time, id, avatarSrc, isRead);
  }

  function appendImageMsg(filename, isMine, time, id, avatarSrc, isRead = false) {
    const imageHtml = `<div class="bubble ${isMine ? 'mine' : 'theirs'}"><img decoding="async" loading="lazy" class="js-lightbox-image" src="index.php?asset=chat&f=${encodeURIComponent(filename)}" alt="Chat image"></div>`;
    insertMessageRow(imageHtml, isMine, time, id, avatarSrc, isRead);
  }

  function appendVoiceMsg(filename, duration, isMine, time, id, avatarSrc, isRead = false) {
    const voiceHtml = `
    ...
    try {
      ...
      if (data.messages && data.messages.length > 0) {
        data.messages.forEach(m => {
          ...
          const isMine = (parseInt(m.sender_id, 10) === <?= (int)$currentUserId ?>);
          ...
          if ((m.message_type || 'text') === 'image' && m.media_file) {
            appendImageMsg(m.media_file, isMine, time, m.id, avatar, !!m.is_read);
          } else if ((m.message_type || 'text') === 'voice' && m.media_file) {
            appendVoiceMsg(m.media_file, m.media_duration || 0, isMine, time, m.id, avatar, !!m.is_read);
          } else {
            appendTextMsg(m.message, isMine, time, m.id, avatar, !!m.is_read);
          }
        });
        scrollBottom();
      }
      ...
      pollDelay = hasNew ? 5000 : Math.min(Math.round(pollDelay * 1.5), 20000);
    } catch (e) {
      console.error('Polling failed', e);
      pollDelay = Math.min(Math.round(pollDelay * 1.5), 20000);
    } finally {
